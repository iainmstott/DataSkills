...
to diverge for different programmes. This allows programs to learn 
...
Within the text you will find a diversity of content, which we'll go through 
...
We are learning how to visualise, manage and analyse data in `R`. Naturally, 
these tutorials will involve R code! R often seems scary when you first 
encounter it, but as we'll see in a bit, you can think of it like a language 
and therefore practise makes perfect. R code is always written in a `fixed width font`, 
where every character has the same width. It's easier to read this way as the 
characters align from line to line.  
\ 

##### **INLINE CODE**

Sometimes when referring to something from R such as a function, 
object or argument (more on that later), `inline code` will be used.  
\ 

##### **CODE CHUNKS** 
More often, code is in _chunks_ which consist of one or more lines of code that 
achieve a certain goal or give a certain output. Here is an example of a code 
chunk and some common outputs:  
...
There are a couple of output types here:

* data, shown as text, is something you either calculate in R or read into R from 
data files.
* plots are images you calculate from your data in R.  
...
Instructive pictures and videos are used where appropriate to help understand 
concepts and content.  
\ 

##### **PICTURE**
Here's a GIF of the _Datasaurus rex_ (and we'll be coming back to her later):  
\ 
...
##### **VIDEO**
Here's a video of what _not_ to do when R just won't do what you ask (and I 
highly recommend you walk away from the computer if you feel yourself getting to 
this point...)  
\ 
...
These content types are a really important content type, as they will test your 
knowledge as  you work through these worksheets, and therefore inform your learning 
at home.  
\ 

##### **QUIZZES**

Quizzes are multiple choice questions. They may have single answers or multiple 
possible correct answers, as seen below.
...
Exercises involve you correcting or typing your own R code, which you can run 
to see a result, or an error, depending on whether you have it correct! These 
will be based on content in the worksheets.  

_By collating all the correct answers to the exercises in a single R script_
_(you'll learn how to do this in the R Basics tutorial), you will end up_
_with a single document that performs a full analysis of the data._
\ 
...
the right answer. If that's the case, it goes without saying to try to get to the 
right answer first. Practice, practice, practice is the key.  
\  
...
# draw random numbers
new_random <- rnorm(1000, , 12)

# mean
mean_average(new_random)

# standard deviation
sd()
```
```{r exampleE2-hint}
Look back at the example code chunks above and spot the similarities and differences to help you correct the code.
```
```{r exampleE2-solution}
# draw random numbers
new_random <- rnorm(1000, 42, 12)
...
and statistics. Most chapters will also use a web application I wrote to help teach 
these things in R. It is available at 
...
real biological data to apply the concepts and skills you learn in part 1. 
These will all be collated in an R file which will take you through the whole 
analytical process, from start to finish!  
\  

Speaking of the analytical process...  
...
We will be using a program called **RStudio** (the technical term for it is an 
"Integrated Development Environment" or _IDE_), to "code" in a _programming 
language_ called **R**. The very first thing you need to do is install both of 
...
##### **R**
_Think of R as a language_ you're writing in, and 
...
##### **RSTUDIO**
RStudio then enables you to write R code, run it, and view the outputs of 
your hard labour!  
\ 
...
Every tutorial will include a list of helpful resources at the top of the page. 
These resources will help you get to grips with R and RStudio. Even the most 
experienced of us still make the simplest of mistakes (trust me, I really do... 
all the time), and having the resources to refer back to is incredibly useful.  

It's also worth knowing that there are _loads_ of resources on the web, 
from forums, through blog posts, to free books. Every programmer uses these, 
all the time. They can be confusing at first, and as there's a multitude of 
ways to do any one thing, you may find diverse advice. It's all part of 
programming! As long as you get the right outcome, it often doesn't matter 
what code you use to get there.  
\ 
...
Create a .R file in the folder you wish to use as your working directory by 
...
Then save this file wherever you like.  

_This is the file in which you're going to write code to fulfil the tasks in 
Part 2 of each tutorial._  
...
```{r, knit, child=c('01_Intro.Rmd', '02_Rbasics.Rmd', '03_SingleVariablePlots.Rmd', '04_DataHandling.Rmd', '05_TwoVariablePlots.Rmd')}
```



<!--chapter:end:Zbook.rmd-->
