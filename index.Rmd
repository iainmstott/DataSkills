...
Within the text you will find a diversity of content. 

* Of course, there is **R code**, which is alway written in a `fixed width font`
such as Courier New, Consolas or Lucida Console. 
  + Code is usually in code _chunks_ 
  which have outputs such as text and plots.
  + _comments_ (notes) on the code are written on 
  lines beginning with `#`.
  + Here's an example of what a code chunk and its outputs looks like:
```{r}
# create an object which is 1000 random 
...
# distribution (should be approximately 1)
sd(random)
```
\  
...
* There are **interactive quizzes** which help you to evaluate your knowledge of 
...
topics and skills learned in Part 1.
* **Part 3** of each chapter presents some analytical tasks for biological data 
for you to code in R.  
\  

Classwork (online) will be done in tutor groups. Homework can be completed 
individually or in tutor groups, as you wish.  

_Parts 1 and 2 of each chapter are to be completed at home each week, prior to the_
_online session._ These should take 1-2 hours per chapter, although this will 
definitely vary depending on the topic.  

_Part 3 is to be completed during the online sessions_, which each last 1 hour.  
