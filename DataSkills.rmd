...
#### **1.4.1 FIRST half of the course (weeks 1-6)**
\ 
...
#### **1.4.2 SECOND half of the course (weeks 8-16)**
\ 
...
#### **1.4.3 General Linear Model (GLM)**
...
Below is a video I made to clarify how the `ggplot2` coding and layers work in practice, in RStudio.
\

<center>
![](https://youtu.be/9zfva5vqq4g){width="80%"}
</center>  
...
<center>
![The analytical process and its feedbacks](www/Process_2_Plot.png){width="80%"}
</center>  
\  
...
We'll be using the [**ShinyGLM**](https://iainmstott.shinyapps.io/shinyglm) webapp 
...
Head on over to [**ShinyGLM**](https://iainmstott.shinyapps.io/shinyglm). 
I assume you've read all of the user guide now, as suggested in Chapter 2½... I know 
it must have been thrilling reading.  

USING THE [**SHINYGLM APP**](https://iainmstott.shinyapps.io/shinyglm):

* Go to the DATA DISTRIBUTIONS page.
* Choose the `eyesight` variable.
...
IN THE [**SHINYGLM APP**](https://iainmstott.shinyapps.io/shinyglm): 
...
Remember that to access help pages for functions, you use `?function_name`. Running the code `?geom_histogram` will bring up a help page for the `geom_histogram` function, which you can also find [HERE](http://search.r-project.org/library/ggplot2/html/geom_histogram.html), and which may help you answer the questions below.
\
...
Answer the questions below about where the mistakes are found. If you're not sure, then you can always edit the options in the [**ShinyGLM app**](https://iainmstott.shinyapps.io/shinyglm) and compare the code above with the code in the app to spot the differences.  
...
```{r ex354_4, echo=FALSE, exercise.eval = TRUE}
hist <- ggplot(data, aes(x = countries)) +
    geom_histogram(aes(y = ..count..), binwidth = 1, fill = 'chocolate4') +
    xlim(0, 30) +
    theme_minimal()
...
* loading `.csv` files into `R` (reminder from Ch2)
* plotting variable distributions using `ggplot`
  + histograms
  + density plots
  + grammar of graphics
...
Remember there's the [ShinyGLM](https://iainmstott.shinyapps.io/ShinyGLM) app to help you out.  
...
1. Read in the data (this was covered in the workshop tasks last week)
2. Plot a histogram of the `DNAmAge` variable. You should:
  ...
  * **iv.** The histogram should have bars which are the colour `"mistyrose4"`
...
3. Plot a density plot of the `Age` variable. You should:
...
1. Read in the data (this was covered in the workshop tasks last week)
2. Plot a histogram of the `TotalScore` variable. You should:
  ...
  * **ii.** Specify x-axis limits of 8 and 21.  
    ...
    - What is the name of a distribution that approximates the variable?  
  * **iii.** The histogram should be a count rather than a density
  * **iv.** The histogram should have bars which are the colour `"orangered"`
    - Search online: can you find any ways to choose a colour other than by specifying a colour name?
      + Hint: in graphic design, colours are commonly specified using amounts of Red, Blue and Green in the colour. This is sometimes coded as what's called a hexademical format.
  * **v.** The histogram should have a 'classic' theme.
  * **vi.** Find the mean and standard deviation of the variable.
...
3. Plot a density plot of the `LitterSize` variable. You should:
...
1. Read in the data (this was covered in the workshop tasks last week)
2. Plot a histogram of the `MaxLongevity` variable. You should: 
  ...
  * **ii.** Work out the number of days there are in 60 years and specify x-axis limits of 0 and 60 years.
    ...
    - What is the name of a distribution that approximates the variable?  
  * **iii.** The histogram should be a count rather than a density
  * **iv.** The histogram should have bars which are the colour `"indianred"`
    - Search online: can you find any ways to choose a colour other than by specifying a colour name?
      + Hint: in graphic design, colours are commonly specified using amounts of Red, Blue and Green in the colour. This is sometimes coded as what's called a hexademical format.
  * **v.** The histogram should have a 'classic' theme.
  * **vi.** Find the mean and standard deviation of the variable.
...
3. Plot a density plot of the `LitterSize` variable. You should:
  ...
  * **i.**  run this code: `LitterSizeRound <- round(data$LitterSize, 0)` to create a new variable which is `LitterSize` rounded to the nearest integer. (Your data frame may be called something other than `data`)
...
We've also touched on the concept of __Tidy Data__. Tidy data allows you, as analyst padawans, 
to easily subset and manipulate data, so you're working with the data that you 
need to be working with. REMEMBER:
...
plotting and analysis. We'll be covering:

* 4.1 Resources
* 4.2 `R` packages
* 4.3 Tidy data
* 4.4 Subsetting data
  + 4.4.1 `slice()`: choosing rows by their number
  + 4.4.2 Pipes `%>%` to chain commands
  + 4.4.3 `select()` to choose columns by number and name
  + 4.4.4 `filter()` to choose rows by logical values
* 4.5 Adding variables
  + 4.5.1 `mutate()` to add variables to a data frame
...
<center>
![The analytical process and its feedbacks](www/Process4.png){width="80%"}
</center>  
\  
...
<!-- ### *** Pages etc. -->
* [_**R for Data Science**_](https://r4ds.had.co.nz/) by Garrett Grolemund and Hadley Wickham, chapter 5 "Data transformation".
* [_**Getting Started with R: An Introduction for Biologists (2nd ed)**_](http://r4all.org/books/gswr2/) 
...
* [**R for beginners**](https://education.rstudio.com/learn/beginner") by RStudio
* [**ShinyGLiM**](https://iainmstott.shinyapps.io/shinyglm)  
\
...
Each column is a variable. Each row represents a single _observation_. That is, one number of a numeric variable, or one category of a factor variable.
\

Now let's consider the _games_ data. As it's a large-ish data frame, we use `head(data)` to look just as the first few rows.
...
The rows represent the level at which the variables are replicated, and so is also often called (especially in experimental biology), a _replicate_. In this case, this is a single person: one data point is measured for each variable, for each person.  
\
...
Once again, we'll use the [SHINYGLM APP](https://iainmstott.shinyapps.io/ShinyGLM) to help us learn how to subset data in an interactive way

* select options within the app to get the output you want
* the CODE tab shows you how you would use `R` code to subset the data with your chosen options. 
...
<center>
![subsetting rows](www/verbs.png){width="80%"}
</center>  
\  
...
Below is a video walkthrough of how to use `dplyr`. This includes information on how a function called a _pipe_ `%>%` is used to chain subsetting commands. The next few sections contain more detail, including quizzes and exercises to practice using the package.  
\

<center>
![](https://youtu.be/GFY33Jt2Od8){width="80%"}
</center>  
...
* **creating subsets of data** to visualise and analyse (e.g. only students, or only near-sighted people, in the _games_ data)
...
* **transposing or transforming variables** where necessary (e.g. as we'll see later, `countries` is a count variable, and we might need to analyse `log(countries)`)
\
...
<center>
![subsetting rows](www/filter.png){width="60%"}
</center>  
\  
...
_**IN THE [SHINYGLM APP](https://iainmstott.shinyapps.io/ShinyGLM)**_:  
...
quiz(caption = "**< QUIZ 1 >** Subsetting Data - slice",
    ...
    question("The code creates the object `rows` and uses that with `slice()` to subset the data. Based on what we learned in Chapter 2 about specifying numeric vectors, which of the following options would achieve the same result? Select one or more options.",
    ...
    question("Even though the options in the above question require fewer lines of code, why might we want to use the `rows` object anyway?`",
...
<center>
![subsetting rows](www/pipe2.png){width=200px}
</center>  
\  
...
<center>
![subsetting rows](www/pipe1.png){width="60%"}
</center>  
\  
...
```{r ex442_1, exercise=TRUE, echo=FALSE}
dataSubset
...
on an object: we'll see this now as we want to select rows AND columns 
from the data. The `select()` function allows us to do this either using the numbers of the columns, or the names of the columns.  
\

<center>
![subsetting rows](www/select.png){width="60%"}
</center>  
\  

##### **CHOOSING COLUMNS BY NUMBER**

First, we'll choose by column number. Imagine that we don't want the data related to the online games: this is the columns `reaction` and `hearing` (columns 7 and 8).  
\

_IN THE [SHINYGLM APP](https://iainmstott.shinyapps.io/ShinyGLM)_:

* leave your selected row range as 7 and 10.
* pick _"Choose by... Column numbers"_ 
  + change the slider to 1 and 6 (this chooses only columns 1 to 6, therefore removes columns 7 and 8) 
* check the _DATA FRAME_ tab to check you have the right rows and columns
* check the _CODE_ tab to see what's changed. 
\

Your code (excluding reading data) should look like this:
```{r eval = FALSE}
rows <- c(7, 10)
cols <- c(1, 6)
...
An extra line of code has appeared. This uses `select()` to choose columns. The data should now show the lines for group 2 and the columns for everything except `reaction` and `hearing`.  
\

The pipes have become important here. The chunk of code above could be interpreted as:  

_"Take the `data` object [which is the games data frame], and first, use the `slice()` function to subset rows 7 to 10 and second, use the `select()` function to subset columns 1 to 6."_  
\

#### **< EXERCISE 2 >** Subsetting Data - `select()`
_The code as it appears in the ShinyGLM app is found below. Try out:_  

* _Swapping the `slice()` and `select()` lines_
* _Deleting the white space at the start of the `slice()` and `select()` lines_
* _Removing the new line after each pipe `%>%` so that the three subsetting lines are on the same line_

```{r ex443_2, exercise=TRUE, echo= FALSE}
rows <- c(7, 10)
...
Based on what you just tried with that code, answer the following questions:

```{r qz443_2, echo = FALSE}
...
<center>
![subsetting columns](www/select.png){width="60%"}
</center>  
\  

##### **CHOOSING COLUMNS BY NAME**

It's useful to sometimes view certain columns according to their number. Generally, it is better to code by referring to column and row names or values. This is because you might want to:

* Add columns or rows
* Remove columns or rows
* Change the order of columns or rows

And if you refer to column or row numbers, then you would have to change all your code to match the new data frame.  
\

`select()` can also be used to subset according to column _**name**_: _IN THE [SHINYGLM APP](https://iainmstott.shinyapps.io/ShinyGLM)_:

* reset the rows to the highest and lowest values (which selects all rows)
* choose _COLUMNS | Choose by... Variable names_
* delete `reaction` and `hearing`. 
* Note how the code tab changes to select variables by name instead of by number.  
\

The subset part of your code should look like this:
```{}
cols <- c('person', 'role', 'group', 'eyesight', 'continents', 'countries')
...
_Edit the code below to give a data frame which has all columns except `group`. Check the hint if you want a tip on how to do this quickly!_
```{r ex443_3, exercise=TRUE, echo= FALSE}
dataSubset <- data 
select()
dataSubset
```
```{r ex443_3-hint, echo= FALSE}
If you want to choose variables to remove rather than variables to keep, use a minus sign, e.g. select(-variable)
```
```{r ex443_3-solution, echo= FALSE}
dataSubset <- data %>%
    select(-group)
dataSubset
```
...
We encountered **logical** vectors in the R Basics Chapter, and they become really important here. Remember, these are variables which give you `TRUE` or `FALSE` values for a "question" you ask. For example:

* which of these values are greater than 7?
```{r}
1:10 > 7
```

* which of these 20 random binary values is equal to zero?
```{r}
# this generates random values of 0 and 1 with 
# equal probability, like tossing a coin. 
binary <- rbinom(20, 1, 0.5)
binary

binary == 0
```

* which of these colours are in the rainbow?
```{r}
colours <- c("red", "green", "blue", "brown", "pink")
rainbow <- c("red", "orange", "yellow", "green", "blue", "purple")
colours %in% rainbow
```
\

 Logical vectors are great for selecting rows of data. When we were selecting rows 7 through 10, we were effectively selecting only people in Group 2.  
 
 If the number of rows or the order of the rows were to change, then row numbers 7 through 10 may no longer represent Group 2. A better option is to use a logical command `group == G2` and the `filter()` function:
...
Try it yourself: _IN THE [SHINYGLM APP](https://iainmstott.shinyapps.io/ShinyGLM)_:

* Select all columns by resetting the Select Columns by number / name options
* choose _ROWS | Choose by... Variable values_ 
* type `group == "G2"` in the box
* click REFRESH. 

You should have a data frame that includes only Group 2. You could try the same for any variable in the data, like `role == 'student'` or `continents == 1`.

 There is an outlier in the `eyesight` variable, which is evident from the histogram for `eyesight`. We can remove this by excluding any values below this value.  
\

#### **< EXERCISE 4 >** Subsetting Data - `filter()`
_Using `filter()` can be very useful for removing data outliers. The `eyesight` variable has an outlier of 14. Edit the code below to remove the outlier value using `filter()` and an appropriate logical._  

_Then, plot a histogram for the `eyesight` variable using the new data frame (refer to ShinyGLM or Chapter 3 to help out if need be)._
```{r ex444_4, exercise=TRUE, echo= FALSE}
# subset the data
dataSubset <- data %>%
...
hist <- ggplot(dataSubset, aes(x = eyesight)) +
    geom_histogram()
# This is the simplest histogram possible: you might want to plot
...
# thing is to use dataSubset as the data layer and eyesight as the 
# x variable.

```
\

There are more questions on logicals below, as they come in many different forms. These questions are based on the `>`, `>=`, `<`, `<=`, `==`, `!=` and `%in%` logical commands introduced in Chapter 2. 
\

```{r qz444_3, echo = FALSE}
quiz(caption = "**< QUIZ 3 >** Subsetting Data - `filter`",
    question("How would you choose rows for every group _except_ group 2?",
    ...
    question("How would you choose rows only for short-sighted people (with negative values for `eyesight`?) Select all that apply.",
        answer("`eyesight < 0`", correct = TRUE),
        answer("`0 > eyesight`", correct = TRUE),
        answer("`eyesight <= 0`"),
        answer("`eyesight != 0`"),
        message = "logicals often work both ways round. In this case there are two correct answers.",
        random_answer_order = TRUE,
        allow_retry = TRUE
    ),
    question("How would you choose rows only for people that have been abroad at some point in their lifetimes? Select all that apply.",
        answer("`countries >= 2`", correct = TRUE),
        answer("`countries > 1`", correct = TRUE),
        answer("`1 < countries`", correct = TRUE),
        answer("`countries <= 2`", message = "careful whether you need to use 'greater than' or 'greater than and equal to'."),
    ...
    question("How would you choose only people who aren't adults?",
        ...
        answer("`role = c('child', 'baby')`", message = "remember the `c()` function 'glues' bits of information together"),
        answer("`role == 'child', 'baby'`", message = "Clue: we might read this code as 'the role is found IN one of: child, or baby'."), 
        answer("`role == 'child' & 'baby'`", message = "Clue: we might read this code as 'the role is found IN one of: child, or baby'."),
        random_answer_order = TRUE,
        allow_retry = TRUE
    )
...
<center>
![subsetting rows](www/mutate.png){width="60%"}
</center>  
\  
...
_IN THE SHINYGLM APP:_

* Reset rows and columns to include all rows and columns.
* Select _ADD A VARIABLE..._: this allows you to add a variable to the data. 
* Choose a variable name for the variable you will create (e.g. `extra_continents`)
* tell `R` how to calculate it, i.e. `continents - 1`. 
...
```{r eval = FALSE}
dataAddVar <- mutate(data, extra_continents = continents-1)
```

You should now see that the variable has been added to the data frame!  

`mutate()` is particularly useful for transforming variables for analysis: often we need to log-transform using `log()`, square root transform using `sqrt()`, or something similar, and `mutate()` lets us easily add that data to the data frame.  
\
...
```{r ex451_5, exercise=TRUE, echo= FALSE}
dataSubset <- data %>%

dataSubset
```
```{r ex451_5-solution, echo= FALSE}
dataSubset <- data %>%
...
You can use `mutate()` as many times as you like to add as many variables as you like (though it only works once in the ShinyGLM app).  
...
_Subset the data frame to include only data for students, remove all columns except `person`, `role`, and `reaction`, and add a variable (with a name of your choice) which represents `reaction` as a percentage rather than a proportion._
```{r ex46_6, exercise=TRUE, echo= FALSE}

```
```{r ex46_6-solution, echo= FALSE}
dataSubset <- data %>%
...
# the order of the mutate and select lines matter, as we have to create
# reaction_pc before we select it.
...
There are many tutorials and worked examples out there for the `dplyr` package. Here are a few I recommend:

* [Introduction to `dplyr`](https://cran.r-project.org/web/packages/dplyr/vignettes/dplyr.html) by the package authors themselves. Works with an example _Star Wars_ data set which is included with `R`, which has information on 87 of the main Star Wars characters (e.g. body mass, height, hair colour, sex, gender...)
* [Datacarpentry's `dplyr` tutorial](https://datacarpentry.org/R-ecology-lesson/03-dplyr.html), which works with a data set of animal diversity and anatomy at a field study site
* The _Data manipulation with `dplyr`_ chapter of this ["Data Carpentry"](https://datacarpentry.org/2015-07-22-JamesMadison/etc/r-datacarpentry.html) tutorial. This also goes over some basic things we covered in Chapter 1 and 2, which may be useful.
...
During this week's workshop, we're going to work with the biological data frames and select the data we want to work with:
...
You may want to remind yourself of `logical` vectors from Chapter 2. These are probably the most useful and important means of subsetting data.
...
You should be able to get through at least Tasks 1-3 in the workshop: task 4 is very useful to complete if you have extra time left, and/or for practise in between online sessions.  
...
1. Load in the `dplyr` and `ggplot2` packages.
2. Set the working directory to the folder your the data is stored in, and read in the data. Ideally you should be continuing in the same `.R` file you created for the Data Distributions Workshop, in which case you will already have code to read in the data anyway.  

The cell ageing data collected data from both men and women. Sex and gender dimensions are incredibly important in any human-based research and clinical trials. In terms of sex (i.e. biological differences arising from chromosomal differences), disease and pharmacology can function differently in men and women. There is a historical bias towards including primarily cisgender, white men in clinical trials and underrepresenting women, trans and nonbinary people, and ethnic minorities. There are many examples of this underrepresentation in [Liu & Mager (2016)](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC4800017/), and a general discussion of historical sex and gender biases, and the importance of sex and gender, in [Pinn (2011)](http://biblioteca.asmn.re.it/allegati/sexandgenderfactorsinmedicalstudies_120615110056.pdf).

3. We'd like to know how many men and how many women were included in the cell ageing study.
  * **i.** `Gender` is the variable representing male / female. This variable should be a factor, if it isn't already. Find out the class of the variable `Gender`. 
    + What is the variable's class?
  * **ii.** Spoiler alert: it isn't a factor. Using the functions we've learned in Chapter 4, make a new data frame with: 
    + a new variable added called `Sex` which is the `Gender` variable converted to a factor
    + what `dplyr` function do you use to add this variable?
    + remove old variable `Gender`.
    + what `dplyr` function do you use to remove this variable?
  * **iii.** Find out how many of each male/female there are. Don't overthink this: it's a simple function you encountered in Chapter 2 which summarieses the variable.
    + How many male, and how many female?
    + Are women over- or under-represented?

It is not fair to make general conclusions about biological phenomena when there isn't fair representation of both sexes. I would personally suggest it is more transparent in this case to remove female data and be honest that the data can only be used to make conclusions relating to men. (This doesn't change the fact that the data collected is inexcusably biased in the first place!!)

  * **iv.** Create a subset of the data frame, using `dplyr` functions and a correct logical query, which only includes the male data.
    + which `dplyr` function do you use to filter the rows of data in this way?
    + how many rows and columns does your new data frame have? Find out one or more `R` function(s) that would tell you the numbers of rows and columns of your data frame.

4. If you have time, it is worth re-plotting the histogram from last week for `DNAmAge` now that you have your data subset.
...
1. Load in the `dplyr` and `ggplot2` packages.
2. Set the working directory to the folder your the data is stored in, and read in the data. Ideally you should be continuing in the same `.R` file you created for the Data Distributions Workshop, in which case you will already have code to read in the data anyway.  
3. The `TotalScore` variable is the one we're interested in, and it's a composite of the `StructureScore`, `ResourceScore`, `QualityScore`, and `DisturbanceScore` variables. 
  * **i.** create a data subset with these four redundant variables removed.
    + which of the `dplyr` functions do you use to select the correct columns?
  * **ii.** `LitterSizeCategory` and `Habitat` should be factor variables. Find out the class of these two variables. 
    + What are the variables' class?
  * **ii.** Spoiler alert: they aren't a factors. Using the functions we've learned in Chapter 4, make a new data frame with: 
    + new variables added called `LitterSizeCategory_fac` and `Habitat_fac` which are the variables converted to factors.
    + which `dplyr` variables did you use to add these variables?
    + remove the old non-factor variables.
    + which `dplyr` function did you use to remove these variables?
  * **iii.** Find out how many rows of data for each Habitat there are. Don't overthink this: it's a simple function you encountered way back in Chapter 2 that summarises the variable.
    + How many of each Urban, Semi and Rural? Are numbers balanced?
  * **iv.** We're interested in how the habitat affects the den score. But there are other, possibly confounding, variables that we should be aware of. In particular, we want to know in which years the different data were collected. Using the `dplyr` functions encountered in this chapter, create 3 separate data subsets for each of the `Rural`, `Semi` and `Urban` rows, using correct logical queries.
    + which function do you use to filter the rows in this way?
    + which years were data for each of the 3 habitats collected in?
    + are the data balanced between the three habitats, in terms of number of data points and when the data were collected?

4. If you have time, plot 3 separate histograms for `TotalScore` separately for rural, semi and urban habitats.
...
1. Load in the `dplyr` and `ggplot2` packages.
2. Set the working directory to the folder your the data is stored in, and read in the data. Ideally you should be continuing in the same `.R` file you created for the Data Distributions Workshop, in which case you will already have code to read in the data anyway.  
3. We're interested in the `MaxLongevity` variable. As we encountered in the last workshop, it's expressed in days. 
  * **i.** Some of the carnivore families are aquatic, and some are terrestrial. Run the code below to create a new variable that tells you whether the species is aquatic or not. 

```{r, eval = FALSE}
Aquatic_Fam <- c("Odobenidae", "Otariidae", "Phocidae")
dataNew <- data %>%
    mutate(Aquatic = Family %in% Aquatic_Fam)
```
\

  * **ii.** add a new variable called `MaxLongevity_yrs` which is the Max Longevity converted to years. Remove the original variable `MaxLongevity` from the data frame.
  * **iii.** Find out how many rows of data (i.e. how many species) there are for aquatic, and for terrestrial species. Don't overthink this: it's a simple function we encountered in the R Basics chapter that summarises the variable.
    + How many aquatic versus terrestrial are there? Are the numbers balanced?
  * **iv.** It's possible that, depending on how we think maximum longevity is related to other life history variables, we might want to analyse terrestrial and aquatic variables separately. Using the `dplyr` functions encountered in this chapter, create 2 separate data subsets for each of the aquatic and terrestrial data rows, using correct logical queries.
    + which function do you use to filter the rows in this way?
4. If you have time, plot 2 separate histograms for `MaxLongevity_yrs` separately for aquatic and terrestrial species.
...
* 5.1 Resources
* 5.2 R packages
* 5.3 Variables: it's all in a name
  + The x / independent / explanatory variable
  + The y / dependent / response variable
* 5.4 Plotting y against a continuous x variable
  + 5.4.1 scatter plots using `geom_point()`
* 5.5 Plotting y against a categorical x variable
  + 5.5.1 box plots using the function `geom_boxplot()`
  + 5.5.2 mean and standard deviation using the function `geom_linerange()`
  + 5.5.3 violin plots using the function `geom_violin()`
* 5.6 An introduction to layering plots
* 5.7 Predicted values and residuals
* 5.8 Other resources
* **< WORKSHOP >**  
\

The hard part of these plots is past already, which is being introduced to `ggplot`. All we're doing now is swapping out graphics layers (specifically _geometries_): everything else pretty much stays the same. You can copy-paste your code from Chapter 3, swap out a couple of things, and BOOM... awesome new plot.  
\

This Chapter re-visits the **PLOTTING** part of the analytical process, once we've visualised data distributions and done any data tidying we need to.  
\
...
* [_**R for Data Science**_](https://r4ds.had.co.nz/) by Garrett Grolemund and Hadley Wickham, Chapter 3, "Data visualisation".
* For the final section on predicted values and residuals, [_Statistical Modeling: A Fresh Approach (2nd ed)_](https://dtkaplan.github.io/SM2-bookdown/) by Daniel Kaplan. Chapters 3 and 4 and sections 6.1, 7.1, 7.3 and 7.5. 
* [_**Getting Started with R: An Introduction for Biologists (2nd ed)**_](http://r4all.org/books/gswr2/) 
by Andrew Beckerman, Dylan Childs, and Owen Petchey, pp 79 - 82, Chapter 4, “Visualizing your data”, Chapter 4.5, "Saving your graphs for 
presenatation, documentation, etc.".  
\ 
...
As in previous chapters, we'll be using the [**ShinyGLM**](https://iainmstott.shinyapps.io/shinyglm) webapp. In this Chapter, we'll mainly be working with the _DATA VISUALISATION_ tab.  
...
In this Chapter we will need to use an R package we already encountered when visualising data distributions.

 * `ggplot2`, is an `R` package which makes pretty pretty data visualisations.

Remember that when working in RStudio, the package first needs to be installed onto your computer. Until you update `R` for the first time, this only needs to be done _once_. 
...
Remember that once a package is installed, it is available for `R` to use. But unless you tell `R` to actually _use_ the package then it won't know it needs to. Therefore you need to tell `R` to use the package each time you restart RStudio. You do this using the `library()` function.
...
As scientists, what we really want to know is:
  
  * how a variable we're interested in is affected by one or more other variables.
  * Whether the relationships between the variables are large or small
  * Whether the relationships between the variables are significant or insignificant
  * Whether the relationships between the variables capture lots or little of the data variation. 
  
We have an expectation of what those relationships look like, which depends on our hypothesis.  
\

We're now at the point where we're plotting such relationships between variables, and linking those plots to hypotheses. There are lots of different ways of phrasing "which variable is which" when it comes to these plots and the statistical analyses we'll encounter later on. Let's quickly demystify a few of these terms.  
\

##### **X / INDEPENDENT / EXPLANATORY**

This variable is the one that, when it comes to our data visualisation and analysis, is doing the influencing. It is generally somehow under our control, whether because we manipulate it experimentally, or because we design our data collection around it: 

* In a lab experiment it is the variable we would choose to manipulate
* In a field experiment we might design our sampling regime to cover the whole range of this variable
* When designing surveys we may choose try and get a representative sample of respondents to cover the whole range of this variable
* When working with open data we would try and make sure we include data which cover the whole range of this variable.  
\

_It's referred to as the **x variable** because it's commonly put on the x axis_. That may be the case for a scatterplot, but is not _always_ the case: e.g. boxplots can be horizontal as well as vertical.  

_It's referred to as the *independent variable** because it is the one that we somehow control_. This however is easily confused with a different concept of _nonindependent data_.  

**We will generally call it the _EXPLANATORY VARIABLE_** in this workbook, as according to the hypothesis it "explains" the y-variable.  
\

We will generally plot the explanatory variable on the x axis.  
\

##### **Y / DEPENDENT / RESPONSE**

This variable is generally not under our control, and is the one we measure or observe, are it's the one in which we're interested to see change.  
\

* _It's referred to as the **y variable** because it's commonly put on the y axis_, but as we've discussed already, that's not always the case.  
* _It's referred to as the **dependent variable** because according to the hypothesis its value is dependent on the explanatory variable._  
* **We will generally call it the _RESPONSE VARIABLE_**, as its value depends on how it responds to the explanatory variable.  
\

We will generally plot the response variable on the y-axis.  
\

Two important points about the response (y) variable:

* The response (y) variable, for the purposes of this module, will always be **numeric**. Categorical response variables do exist, but their analysis is difficult. 
* There will only ever be one response (y) variable in this module. Analysing mutliple response variables in a single analysis is possible, but again, difficult.  

$~$

##### **LINKING HYPOTHESES AND EXPLANATORY / RESPONSE VARIABLES**

It's important to be able to define your explanatory and response variables from your hypothesis. Based on what's said above, here are some practice questions relating hypotheses and variable types.  

```{r qz53_1, echo=FALSE}
quiz(caption = "**< QUIZ 1 >** Linking variables and hypotheses",
    question("Your hypothesis is that people in coronavirus lockdown who have more green space around them are happier because they have more opportunity to get outside and to exercise. What is the explanatory variable?",
        answer("Amount of green space", correct = TRUE),
        answer("Happiness"),
        answer("Amount of time in lockdown"),
        answer("Amount of exercise"),
        answer("Amount of time spent outside"),
        random_answer_order = TRUE,
        message = "The explanatory variable is green space, and the response variable is happiness. Exercise or time outdoors might be the mechanism by which happiness is achieved, but if they were our explanatory variable(s) then suddenly the question becomes different: it's about how exercise and time outdoors affect happiness."
    ),
    question("Your hypothesis is that horses kept in herds show fewer signs of stress because they feel safer than if they're kept alone. What is the explanatory variable?",
        answer("Whether or not the horse is kept in a herd", correct = TRUE),
        answer("The number of other horses a horse is kept with"),
        answer("The number of signs of stress a horse shows"),
        answer("The amount of time the horse is kept alone"),
        answer("The number of stressful situations a horse is subjected to"),
        random_answer_order = TRUE,
        message = "The explanatory variable is whether or not a horse is kept in a herd. The hypothesis mentions nothing about the number of horses, and if this were the explanatory variable the the hypothesis becomes a little different: stress would be presumed to depend specifically on the number of horses a horse is kept with."
    ),
    question("Your hypothesis is that the frequency of a gene coding for breast cancer susceptibility is linked to the average age of mothers in a population, because mutation rates are higher in older ova in older mothers. What is the response variable?",
        answer("Frequency of the gene", correct = TRUE),
        answer("Average age of mothers in each population"),
        answer("Age of ova"),
        answer("Mutation rates in the population"),
        answer("Breast cancer rates in the population"),
        random_answer_order = TRUE,
        message = "Gene frequency is the response variable as that's what you're measuring. Frequency of breast cancer is not necessarily the same as gene frequency. The explanatory variable is the average age of mothers."
    )
)
```
...
***
### **5.4 PLOTTING RESPONSE (Y) VARIABLES AGAINST NUMERIC EXPLANATORY (X) VARIABLES**
\

We used `ggplot` to plot histograms. Histograms only show one variable from the data, on the x axis. That's why we only specify only an x variable using the _aesthetics_ function `aes()` when creating the histogram ggplot. The y variable is simply a count or a density. Take a look to remind yourself of how a histogram is plotted:  
\

```{r}
# Histogram for eyesight
hist <- ggplot(data, aes(x = eyesight)) +
    geom_histogram()
hist
```
\

To create a plot with **_both an explanatory and response variable_**, we minimally need to change two things from the histogram code.

1. You need to specify two variables in the _**aesthetics**_: both x and y. In histograms, we only specified x.
2. You need to specify a different _**geometry**_: we're not plotting a histogram any more.
\

For comparison to the above histogram code, here's code which would plot a very simple scatter plot with `eyesight` as the explanatory (x) variable and `reaction` as the response (y) variable. Spot where those two changes are made to the aesthetic layers and the geometry layers compared to the histogram:  

```{r}
scatter <- ggplot(data, aes(x = eyesight, y = reaction)) +
    geom_point()
```
\

**REMEMBER** that you can use the [ShinyGLM app](https://iainmstott.shinyapps.io/ShinyGLM) to help find out some (but not all) answers if you need to. On the DATA DISTRIBUTIONS page, choose the options you need, and then look at the CODE tab to see how to code the options you've chosen. As a reminder, we have already encountered these options during Chapter 3, which are:

* changing DATA when setting up `ggplot()`
* changing aesthetics using the `aes()` function when setting up `ggplot()`
* changing fill colours of bars using the `fill` argument in `geom_histogram()`
* changing axis limits using `xlim()` and `ylim()`
* changing axis labels using `xlab()` and `ylab()`
* changing the theme using e.g. `theme_minimal()`

$~$

### **5.4.1 `geom_point()`: SCATTER PLOTS**
\

We might hypothesise that people who have been to more continents have also been to more countries, because they have had the opportunity to visit more countries across the world. First step to seeing if this is true is to plot the variables. Think in these steps:

* Which is the explanatory variable, and which is the response? Put these in the right places in the aesthetics.
* The code just above shows you how to plot a basic scatter plot. Pick the right geometry function from that code (remember: these begin with `geom()`)
* Add in the rest of the requirements listed below in almost the same way as you would for all the plots you've made before this
* If you're stuck or something isn't working then you can find most solutions by trying to recreate the plot in ShinyGLM and seeing how the CODE tab has changed.
\

#### **< EXERCISE 1 >** Scatter plots
_Edit the code below to plot a scatter plot which:_

1. _has point colour to `slategrey4`_
2. _has larger point size (you may have to search how to do this; if not there's a hint)_
3. _has point shape `+` instead of circles._
4. _Has x label "Continents" and y label "Countries"_
5. _has a minimal theme_

```{r ex541_1, exercise=TRUE, echo=FALSE}
scatter <- ggplot(data = data, aes(x = , y = )) +
    geom_()

scatter
```
```{r ex541_1-hint}
You can increase the point size using the 'size' argument in the geom_histogram() function.
You can change point shape by using the 'shape' argument in the geom_histogram() function.
```
```{r ex541_1-solution}
scatter <- ggplot(data = data, aes(x = continents, y = countries)) +
    geom_point(colour = "slategrey4", size = 3, shape = "+") +
    xlab("Continents") + ylab("Countries") +
    theme_minimal()
...
There are many different ways of showing how response variables are related to categorical explanatory variables. These are effectively just different ways of illustrating the variable's distribution - just like we did in Chapter 3 with histograms - but for several groups of data rows representing individual categories of your x variable. There are three main ways of illustrating distributions:

* Using _**summary statistics**_
  + _Boxplots_ do this with _nonparametric statistics_ (median, IQR), and in `ggplot2` they are specified using the function `geom_boxplot()`
  + _Mean and standard deviation_ are very useful for reporting results from analyses and are the main way you should present categorical data. We use `geom_pointrange()` to plot these.
* Using _**density distributions**_
  + _Violin plots_ do this, showing the continuous density distribution (like we encountered in Chapter 3), but with a twist. They are specified in `ggplot2` using the function `geom_violin()`.

 most commonly using summary statistics, but also certain forms of density plots.

$~$

### 5.5.1 BOX PLOTS USING `geom_boxplot()`
\

It might be reasonable to hypothesise that people with different `role`s have been to different numbers of countries, on average. 

* Children and babies are young, and haven't had time to travel.
* Students are also relatively young on average (and may not have the money to travel),
* Staff and external people are old, wise, and ever so very experienced and worldly (of course). 

Therefore the number of countries visited would be in the order:  

baby < child < student < staff and person.  
\

The geometry function we use for box plots is `geom_boxplot()`. Just like for a simple scatter plot, we can swap out the aesthetics and the geometry to get the plot we need.  
\

Try the same steps as for the previous exercise:

* Which is the explanatory variable, and which is the response? Put these in the right places in the aesthetics.
* Pick the right geometry function from that code (remember: these begin with `geom()`)
* Add in the rest of the requirements listed below in almost the same way as you would for all the plots you've made before this
* If you're stuck or something isn't working then you can find most solutions by trying to recreate the plot in ShinyGLM and seeing how the CODE tab has changed.  
\

#### **< EXERCISE 2 >** Box plots
_Your boxplot should have:_

* _A thinner width of boxes than before_
* _Black lines around the boxes_
* _Transparent fill in the boxes_

_You won't have encountered these options before. I suggest looking on the `ggplot` cheat sheet (linked to in the RESOURCES section), find the scatter plot section, and look underneath for the arguments you can change in the `geom_histogram()` function. There's also a pointer in the Hints for the exercise if you need a little more help!_
\

```{r ex551_2, exercise=TRUE}
box <- ggplot(data, aes(x = , y = )) +
    geom_() +
    theme_grey()
box
```
```{r ex551_2-hint}
You can change the line colour using colour = "black"
The transparent fill is achieved using alpha = 0
```
```{r ex551_2-solution}
box <- ggplot(data, aes(x = role, y = countries)) +
...
In fact, the `alpha` argument can be any value between 0 and 1, and it controls the transparency of the colour. We'll see later that this is really useful when layering plots.

$~$

### 5.5.2 MEAN AND STANDARD ERROR USING `geom_linerange`
\

Showing means and standard errors is THE way to present your data, especially when you're reporting results of most statistical analyses (as these usually rely on means and standard errors to calulate p-values).  

To do this with `ggplot`, we have to modify a function we've already encountered. What we do is:

1. Use `geom_point()` to show the mean values for each category
  + We use a couple of extra arguments to do this: `stat = "summary"` to tell `R` that we don't want to plot _all_ points but only a _summary_ of the points, and `fun.y = "mean"` to tell `R` to that the summary we want is a mean of the y values.
2. Use `geom_linerange()` to add error bars
  + use those same extra arguments: `stat = "summary"` to tell `R` that we want a _summary_ of the points and `fun.y = "mean_se"` to tell `R` to that the summary we want is the standard error around the mean values.
  \
...
```{r}
meanSE_plot <- ggplot(data, aes(x = role, y = countries)) + 
...
I can tell you that this is MUCH more complicated to so in base `R`. Knowing the `ggplot` technique will save you a lot of pain in the future.  
\

##### _WHY ARE WE NOT USING BAR PLOTS?_
\

**Bar plots are bad.** They're constrained to have a y-axis limit of 0 (either the lower limit, if the bars are positive, or the upper limit, if the bars are negative). This means that if your response (y) axis values are large and have a small range, you just can't see the differences between them. There's no reason not to use points with standard error bars, as we've seen here, and a sensible y-axis range. In my opinion, even if the bars have a large range and the differences are clearly visible, there's no reason not to use points anyway.  

That's not to say bar plots don't have their place. They're very good for illustrating proportions. Incidentally, don't ever use pie charts to illustrate proportions. They're terrible. First, they have no axis. Second, research shows that people are very bad at interpreting proportions from pie charts. Circles are for biscuits and cake. Preferably biscuits because they're better than cakes.

$~$

### 5.5.3 VIOLIN PLOTS USING `geom_violin()`
\

Violin plots are essentially like mini-density plots (which we encountered when plotting histograms). They show a nice overall shape to the distribution of the response (y) variable, which isn't clear from a boxplot. Many of the same customisations are available, and to use it you just swap `geom_boxplot()` for `geom_violin()`:  

```{r}
viol <- ggplot(data, aes(x = role, y = countries)) +
...
Combining plots is where `ggplot` comes into its own. The first means of combination is _**layering**_. Basically, putting one plot on top of another so you benefit from twice the data viz deliciousness...  

We have actually encountered this before, when we overlaid density plots on histograms.  

For example, we might want to show the whole distribution using a violin plot, but also overlay a boxplot so it's a bit more statistical.  
\

_All we have to do is add another geometry function!_ Trust me, in base `R` something like this is a NIGHTMARE to do.  
\

```{r}
combo_plot <- ggplot(data, aes(x = role, y = countries)) +
    geom_violin(fill = "thistle2", colour = "white", alpha = 0.7) +
...
These plots of explanatory (x) and response (y) variables are a good point to start understanding the statistics you'll use in later Chapters. Plots are a great way to understand what makes for significant relationships between variables. 
\

### PREDICTED VALUES
\

Most statistical analyses will give you a value predicted value for the response (y) variable for each value of the explanatory (x) variable.  

For a **continuous** explanatory (x) variable, this is the line of best fit. This is the classi form of a simple line graph, $y=intercept+slope\times x$:
\

$y = \beta_0 + \beta_1 x$  
\

$\beta_0$ represents the intercept. $\beta_1$ represents the slope.  
\

```{r echo = FALSE}
jitter <- ggplot(data = data, aes(x = continents, y = countries)) +
    geom_point(position = position_jitter(width = 0.1, height = 0, seed = 123),
               colour = "slategrey", size = 6, shape = "+") +
    xlab("Continents") + ylab("Countries") +
    theme_minimal() +
...
For a **categorical** explanatory (x) variable, this will be close to (or the same as) some summary statistic of the variable. Commonly this is the mean of the data for each category:  
\

```{r echo = FALSE}
meanSE_jitter <- ggplot(data, aes(x = role, y = countries)) + 
    geom_point(position = position_jitter(width = 0.1, height = 0, seed = 123),
               colour = "slategrey", size = 6, shape = "+") +
    geom_point(stat = "summary", fun.y = "mean", size=3) + 
    geom_linerange(stat = "summary", fun.data = "mean_se") +
...
The actual value of the data point is the predicted value plus the *residual*:  

DATA = PREDICTED VALUE + RESIDUAL  
\

Therefore the residual is the difference between the data point and the predicted value:  

RESIDUALS = DATA - PREDICTED VALUE  
\

These residuals - the spread of the response variable data around the predicted values - characterise the **ERROR** of the model. This is what determines whether a relationship between two variables is _STATISTICALLY SIGNIFICANT_ or not.  
\

**RESIDUALS ARE THEREFORE VERY IMPORTANT!**  
\

A negative residual is linked to a data point that's below the predicted value. A positive residual is a data point that's above the predicted value.  

Let's look at what they look like:  
\

```{r echo = FALSE}
lm_coef <- lm(data$countries ~ data$continents)$coef
data_predict <- data 
data_predict$predict <- data$continents * lm_coef[2] + lm_coef[1]
cont_resid_plot <- ggplot(data = data_predict, aes(x = continents, y = countries)) +
    geom_linerange(aes(ymin = countries, ymax = predict), 
                   position = position_jitter(width = 0.1, height = 0, seed = 123),
                   colour = "grey50", lty = 3) +
    geom_linerange(aes(ymax = countries, ymin = predict), 
                   position = position_jitter(width = 0.1, height = 0, seed = 123),
                   colour = "grey50", lty = 3) +
    geom_point(position = position_jitter(width = 0.1, height = 0, seed = 123),
               colour = "slategrey", size = 6, shape = "+") +
...
```{r echo = FALSE}
data_predict <- data %>% group_by(role) %>% mutate(mean_countries = mean(countries))
meanSE_jitter2 <- ggplot(data_predict, aes(x = role, y = countries)) + 
    geom_point(position = position_jitter(width = 0.1, height = 0, seed = 123),
               colour = "slategrey", size = 6, shape = "+") +
    geom_linerange(aes(ymin = countries, ymax = mean_countries), 
                   position = position_jitter(width = 0.1, height = 0, seed = 123),
                   colour = "grey50", lty = 3) +
    geom_linerange(aes(ymax = countries, ymin = mean_countries), 
                   position = position_jitter(width = 0.1, height = 0, seed = 123),
                   colour = "grey50", lty = 3) +
    geom_point(stat = "summary", fun.y = "mean", size=20, shape = "-") +
    geom_linerange(stat = "summary", fun.data = "mean_se") +
    theme_minimal()
meanSE_jitter2
```
...
The residuals determine the error variance, which is the variation in the data that the predicted values can't explain. This is the sum of the squared residuals, divided by the number of data points:  

$\sigma^2=\sum_{n=1}^{N}\frac{(y_i-\mu_i)^2}{N}$  

$\sigma^2$ is the error variance, N is the number of data points, $y_i$ are the response (y) variable data points, $\mu_i$ are the predicted values. This is the same as calculation of the overall variance in the response (y) variable, which you should be familiar with, except that we use the predicted values rather than the mean of the response (y) variable.  
\

Take a little time to go over residuals and error variance, because an understanding of these things will help a lot when it comes to understanding and interpreting statistical models.  

$~$

```{r qz552_2, echo=FALSE}
quiz(caption = "**< QUIZ 3 >** Predicted values and residuals",
    question("You are investigating leaf length of different species of plant. If the length of a plant's leaf is 10cm and the mean leaf length for that species is 12.5cm, what is the residual value for that leaf?",
        answer("-2.5cm", correct = TRUE),
        answer("2.5cm"),
        answer("22.5cm"),
        answer("1.25cm"),
        random_answer_order = TRUE,
        message = "The residual is the data point minus the predicted value. The predicted value is the species' mean, so the residual is 10 - 12.5 = -2.5"
    ),
    question("You're investigating the relationship between how much time dogs spend barking per hour, and number of times people walk past their houses. The predicted relationship has an intercept of 2mins, and a slope of 1.4. What is the predicted time barking for time barking when 12 people walk past on average in an hour?",
        answer("18.8mins", correct = TRUE),
        answer("25.4mins"),
        answer("14.8mins"),
        answer("40.8mins"),
        answer("2.5 hours", message = "It's definitely not possible to bark for 2.5 hours per hour!"),
        random_answer_order = TRUE,
        message = "The predicted value is the intercept (2) plus the slope(1.4) multiplied by the x value (12). Therefore the answer is 2 + (1.4 x 12) = 18.8 minutes."
    ),
    question("You're measuring how many times people in the 10 University of Lincoln Biomedical Science tutor groups get ill. In one tutor group, with 8 students, you find that the number of times the students get ill are 1, 3, 4, 2, 2, 0, 2, and 2. What is the residual variance for this group? Hint: find the mean, then calculate the residual values, square each residual value, add them up, and divide by the number of students in the tutor group.",
        answer("1.25 times", correct = TRUE),
        answer("2 times"),
        answer("0.776 times"),
        answer("-1.5 times"),
        answer("1 times", message = "It's definitely not possible to bark for 2.5 hours per hour!"),
        random_answer_order = TRUE,
        message = "mean is 2; residuals are -1, 1, 2, 0, 0, -2, 0, 0; squared residuals are 1, 1, 4, 0, 0, 4, 0, 0; residual variance is 1.25."
    )
)
```
...
The [_**R Graph Gallery**_]() has advice on how to plot gazillions of different plot types, and links to courses in programming plots.  
\

[_**Data to Viz**_](https://www.data-to-viz.com/) is BRILLIANT. It helps learn learning data visualisation, with clear decision trees for choosing the best plots to use. It helps with information on when and why to use different types of plots. It includes a bunch of caveats and pitfalls for data visualisation as well. It doesn't contain any `R` code itself, but it links to `R` code for plotting different graphs.

$~$

### **< WORKSHOP >** Data Visualistion I
\

When creating plots, remember the layers you need, one at a time. 

1. What's the data later (this is the first thing you put in the `ggplot()` function)
2. What's the aesthetics (i.e. the x and y variables)
3. What geometries are you using (e.g. scatter plots, boxplots, etc.)
4. What other ways are we tweaking the plots (colours, axis limits, etc)
5. What theme are you adding?  
\

During this week's workshop, we're going to work with the biological data frames and select the data we want to work with:

* loading `.csv` files into `R` (reminder from Ch2)
* plotting response variables against continuous explanatory variables
  + scatterplots
* plotting response variables against categorical explanatory variables
  + box plots
...
We will also practice some things we encountered during previous sessions:

* data subsets
* basic R  
\

Remember which data you're working with:
...
You should already have done points 1-3 in the last Chapter. If you don't have the workspace or the code already saved, whip through them as fast as possible using the answers for the last Chapter, which are given to you below in steps 1-3.  
\
...
2. Load your data (if you are saving your `R` workspace at the end of each session then this will be loaded already).  

```{r eval = FALSE}
setwd("/path/to/your/file") # Mac
setwd("C:/path/to/your/file")
data <- read.csv("cell_age_data.csv")
```
\

As we saw last week, the data contain only 2 data points for females.  

3. In the last session, we used `dplyr` functions to do the following. If you don't still have this saved in your workspace, quickly complete these tasks (if you haven't done it yet / don't have the code saved, look back at the worksheet answers so you can get this done as fast as possible):

* **i.** Add a new variable to the data frame called `Sex` which is `Gender` converted to a factor, and remove your original `Gender` variable.
...
* **ii.** Remove females from the data as they only comprise two data points (for one individual).

```{r eval = FALSE}
dataMale <- dataSubset %>%
    filter(Sex == "male") # could also use Sex %in% "male"
```
\

There is one `Donor` who had multiple fibroblast samples taken (`Donor92A`). This is called data _pseudoreplication_ or _nonindependent data._ 

4. To remove the extra sample for `Donor92A`:

* **i.** Create a new data frame which has the row containing `SampleNo19` removed from your data.

5. Plot an appropriate plot to show that there is a close relationship between `Age` and `DNAmAge`.

* **i.** Concerning the two variables...
  + Which is the explanatory, and which is the response?
  + Which variable goes on which axis?
* **ii.** Change the plot's theme from the default
  + What is the default plot theme?
* **iii.** It's not always easy to read the exact values of poins on their axes. A "rug" plot can make this a lot easier: it indicates, on each axis, the values that match each data point. 
Find out the `ggplot` function to plot a rug plot, and add it as an extra layer to your plot.  
* **iv.** imagine where the line of best fit would go. Do you think the residuals would be approximately equal across the range of `DNAmAge`? 
  + If not, at what values of `DNAmAge` are the residuals smaller, or larger?
\

**If you have time...**  
\

One of the things we're interested in is whether the age of the fibroblasts matches the age of the converted neuron cells. A first step to finding this out is to plot a boxplot comparing the two cell types.  

6. Overlay a boxplot on a violin plot, with `Tissue` on the x axis and `DNAmAge` on the y axis. 

* **i.** Choose an appropriate colour so that the two are easily visible. Often it's best to: 
  + choose a pale colour for the background (violin plot) and 
...
You should already have done points 1-2 in the last Chapters. If you don't have the workspace or the code already saved, whip through them as fast as possible using the answers for the last Chapter, which are given to you below in steps 1-2.  
\
...
setwd("/path/to/your/file") # Mac
setwd("C:/path/to/your/file")
data <- read.csv("urban_dogs.csv")
```
\

For now, let's say we're only interested in the difference between `Urban` and `Rural` habitats: we don't care about `Semi`-urban habitats. 

3. Create a new data frame which has all the rows that are for `Semi`-urban `Habitat`s removed.
4. Plot a figure to show that there is a difference bete `Habitat` and `TotalScore`. You will plot a boxplot overlaid on a violin plot to illustrate the difference between the two.

* **i.** Concerning the variables:
  + Which is the explanatory, and which is the response?
  + Which variable goes on which axis?
* **ii.** Change the plot's theme from the default
  + What is the default plot theme?
* **iii.** Make sure the plot is easy to read. This may include:
  + a pale colour for the violin plot and its outline
  + a transparent fill for the box plot
  + a narrower width for the box plot
* **iv.** imagine where the means of the two groups might lie: these are the two fitted values for the two groups. Do you think the residuals would be approximately equal across the range of `TotalScore`? 

If you're stuck, check the "Layering Plots" section of Chapter 5: it shows you how to do this.
\

**If you have time...**  
\

We're also interested in whether there is a relationship between `LitterSize` and `TotalScore`. We can explore this using a scatter plot.   

* **i.** Change the point size, shape, and colour.
  + Do you use arguments, functions or objects to do this?
  + There's an issue with plots like this, in that it's difficult to see the data. What is it about the plot that makes the data difficult to see?
...
setwd("/path/to/your/file") # Mac
setwd("C:/path/to/your/file")
data <- read.csv("pantheria_data.csv")
```
\

Last week, we created a variable that told you whether a species is aquatic or not, based on its family.  

3. If you don't still have this saved in your workspace, quickly complete these tasks (if you haven't done it yet / don't have the code saved, run the code you're given below so you can get started on the new stuff as quickly as possible):

* **i.** Add a new variable to the data frame called `Aquatic` which is a logial variable which is `TRUE` if the animal is aquatic, and `FALSE` if not.
* Add a variable `MaxLongevity_yrs` which is the maximum longevity in years, and remove `MaxLongevity`.
* Create two data subsets: one for aquatic and another for terrestrial.
...
dataNew2 <- dataNew %>%
    mutate(MaxLongevity_yrs = MaxLongevity / 365) %>%
        select(-"MaxLongevity")

data_Aqu <- dataNew2 %>%
    filter(Aquatic == TRUE) # could also be Aquatic %in% TRUE
data_Ter <- dataNew2 %>%
    filter(Aquatic == FALSE) # could also be Aquatic %in% FALSE
```
4. Create two plots, one for terrestrial carnivores and one for aquatic carnivores, to show the relationship between `MaxLongevity_yrs` and `GestationLength`.

* **i.** Concerning the two variables...
  + Which is the explanatory, and which is the response?
  + Which variable goes on which axis?
* **ii.** Change the plot themes from the default
  + What is the default plot theme?
* **iii.** It's not always easy to read the exact values of poins on their axes. A "rug" plot can make this a lot easier: it indicates, on each axis, the values that match each data point. 
Find out the `ggplot` function to plot a rug plot, and add it as an extra layer to your plot.  
* **iv.** Does it appear that the relationships between these two variables are different for aquatic versus terrestrial carnivores? If so, what do you see?
* **v.** imagine where the line of best fit would go for each of the plots. Do you think the residuals would be approximately equal across the range of `GestationLength`, in both aquatic and terrestrial species?
\

**If you have time...**  
\

There might be a difference in gestation time for aquatic and terrestrial carnivores. It could be useful to plot a box plot to see the scale of this.  

6. Overlay a boxplot on a violin plot, with `Aquatic` on the x axis and `GestationLength` on the y axis. Think about which data set you'll need: it should have both aquatic and terrestrial carnivores. 
...
This first part of this chapter contains _Quizzes_ and _Exercises_. These multi-
choice questions and `R` coding exercises will test your learning on this chapter's
topic.

The final section of the Chapter contains some tasks for you to complete with a real data set, on the topic of this Chapter. You'll be coding this yourselves in `R` using RStudio.  
\
...
* 6.4 Visualising one numeric and one categorical explanatory variable (scatter plots using with grouped points)
  + 6.4.1 Grouping points by colour
  + 6.4.2 Grouping points by shape
  + 6.4.3 Combining grouping methods
* 6.5 Visualising two categorical explanatory variables: (split box plots, violin plots, means and standard errors)
  + 6.5.1 Using facets to split plots
* 6.6 Two numeric explanatory variables: let's not think about that just yet.
* 6.7 Other resources
* **< WORKSHOP >**  
\

In this chapter we're simply expanding on what we learned in Chapter 5. Although as we encountered in Chapter 5, most hypotheses involve only one response variable, hypotheses may include more than one explanatory variable. To visualise these, we often simply have to add a small piece of code to the existing, familiar `ggplot` structure. This chapter shows what extra pieces of code are needed, and where you put them.  

**This is where `ggplot` really comes into its own compared to base `R` graphics: it's far more complicated to reproduce the plots you'll see in this Chapter using base `R` methods!**  
\
...
* [_**Getting Started with R: An Introduction for Biologists (2nd ed)**_](http://r4all.org/books/gswr2/) 
by Andrew Beckerman, Dylan Childs, and Owen Petchey, pp 79 - 82, Chapter 4, “Visualizing your data”, Chapter 4.5, "Saving your graphs for 
presenatation, documentation, etc." and Chapter 8, "Pimping your plots: scales and themes in ggplot2". 
\ 
...
Hypotheses often (in fact I would say usually) involve more than one explanatory variable. It's important therefore to be able to visualise these when exploring data and evaluating results.  
\

**We will only consider a maximum of two explanatory variables in this course, but it is possible to visualise, and analyse, more than two.**
\

There are three different possible combinations of explanatory variables:
...
##### **6.3.1 UNDERSTANDING HYPOTHESES WITH MULTIPLE EXPLANATORY VARIABLES**
\

Below there are three hypotheses for biological data sets. Read each hypothesis carefully and consider how the description is visualised.  
\

1. Different types of coronavirus vaccine will have different efficacies in the over-65s: RNA vaccines will have a higher efficacy in the over-65s than the under-65s whilst inactivated virus vaccines will have higher efficacy in the under-65s than the over-65s.
* response (y) variable: vaccine efficacy
* explanatory (x) variables
  + age (categorical): over- or under-65
  + vaccine type (categorical): RNA or inactivated virus vaccine  
\

<center>
![Vaccine hypothesis](www/vaccine.png){width="50%"}
</center>  
\  

The two age groups respond differently to the two vaccines. It's not like one vaccine is better than the other across the board; it depends on your age. This is illustrated by the lines linking the two groups showing different 'slopes'.  
\

2. Parasite load is always higher in barn chickens compared to free-range chickens, and increases with chicken age.
* response (y) variable: parasite load
* explanatory (x) variables
  + barn vs free-range (categorical)
  + chicken age (numeric)  
\

<center>
![Chickens hypothesis](www/chicks.png){width="50%"}
</center>  
\  

The increase in parasite load with age is the same wherever the chickens are raised. But, parasite load is higher at all ages in barns compared to free range. This is indicated by the fact that the lines have the same slope.  
\

3. When different species of plant are subjected to increasing durations of direct sunlight, the growth of shade-loving plants decreases whilst the growth of light-loving plants increases.
* response (y) variable: plant growth
* explanatory (x) variables 
  + plant type (categorical): shade- or light-loving
  + duration of direct sunlight (numeric)  
\

<center>
![Plants hypothesis](www/plants.png){width="50%"}
</center>  
\  

The different types of plant respond differently to light: for some, more light is good. For some, more light is bad. Therefore the slopes of the lines are different.  
\

Whether or not the slopes are different is a really important for understanding hypotheses...

##### **ADDITIVE RELATIONSHIPS**

the chickens are an example of an **ADDITIVE** relationship. The effect of rearing space adds to the effect of age but the two effects don't modify each other.

##### **INTERACTIVE RELATIONSHIPS**

Both the vaccine and the plants are examples of **INTERACTIVE** relationships. This means that the effect of one variable depends on the effect of the other. How effective one vaccine is compared to the other depends on what your age is. How plant growth responds to light depends on what type of plant it is. Wherever you can say that the effect of one explanatory variable will depend on another one, we call that an **INTERACTION**.

$~$

***
### **6.4 VISUALISING ONE NUMERIC AND ONE CATEGORICAL EXPLANATORY VARIABLE**
\

We used `ggplot` with `geom_point()` to visualise a numeric explanatory (x) variable against a numeric response (y) variable. The best way to introduce a further categorical variable into the mix is to change the appearance of each point according to the group it belongs to.  
\

As we are introducing a new variable, we need to introduce a new _aesthetic_ with the function `aes()`. The flexibility of `ggplot` means this can be done in several different places in the code, but I like to put it in the `geom_point()` function. You may note differences between some code here and compared to the ShinyGLM app, which reflect certain flexibilities like this.   
\

##### **6.4.1 GROUPING SCATTER PLOT POINTS BY COLOUR**
\

<center>
![Points](www/pointplot_icon.png){width="50%"}
</center>  
\  

You may recall code for the simple scatter plot of reaction against eyesight from section 5.4. In order to colour the points by role, all that is needed is to put `aes(colour = role)` inside the `geom_point()` function:  
\
...
This makes it possible to see which points match which role. Only students have been to 5, 6 or 7 continents. With just the points, it's difficult to understand what this means for the relationship between eyesight and reaction. Adding a fitted line makes this clearer. This means a _**statistics**_ layer to the plot using `stat_smooth()`.  
\

A couple of notes on the code below: 

* `method = "lm"` means a _straight line_ is plotted. `"lm"` stands for "linear model", which is the type of statistical model we'll be using later in the course.
* `se = FALSE` instructs not to plot standard error margins; just the fitted line.
* There are differences between the above code and ShinyGLM, which just reflect the flexibility of `ggplot`. They both give the same output!  
\
...
The fitted line shows a shallow declining relationship overall. Therefore, short-sighted people take longer to react. But is that also the case for each individual group? By specifying the fitted lines to also be coloured by `role`, we can see this:  
\
...
scatter + stat_smooth(method = "lm", se = FALSE, aes(colour = role))
```
\

It seems `staff` and `student` are decreasing, but `person` is increasing. This is something we couldn't have known unless we visualised the data with _both_ explanatory variables.  

$~$

#### **< EXERCISE 1 >** Scatter plots
_Recall the code you used to create a scatter plot with a response variable of countries and an explanatory variable of continents (Section 5.4.1, exercise 1)._

* _Add to this code so that points are coloured by `role`_
* _As with the above eyesight/reaction example, create two plots with fitted lines:_
  + _one with an overall fitted line_
  + _one with a different fitted line to each group_

```{r ex641_1, exercise=TRUE, echo=FALSE}
# solution from 5.4.1 exercise 1
scatter <- ggplot(data = data, aes(x = continents, y = countries)) +
    geom_point(colour = "slategrey4", size = 3, shape = "+") +
    xlab("Continents") + ylab("Countries") +
    theme_minimal()
...
```{r ex641_1-solution}
scatter <- ggplot(data = data, aes(x = continents, y = countries)) +
    geom_point(aes(colour = role), size = 3, shape = "+") +
    xlab("Continents") + ylab("Countries") +
    theme_minimal()
...
To get points with a different shape, simply change the `shape` aesthetic, rather than the `colour` aesthetic:  
\

```{r message = FALSE}
scatter <- ggplot(data, aes(x = eyesight, y = reaction)) +
    geom_point(aes(shape = role))
...
In Section 5.6 we saw that it's possible to layer different types of plot together. Similarly, it's possible to combine different ways of defining points. We could, for example, augment our eyesight-reaction plot so that point size reflects the hearing score, just by adding `size = hearing` into the `aes` of `geom_point()`.  
\

```{r message = FALSE}
scatter <- ggplot(data, aes(x = eyesight, y = reaction)) +
    geom_point(aes(colour = role, size = hearing))
scatter + stat_smooth(method = "lm", se = FALSE, aes(colour = role))
```
...
***
### **6.5 VISUALISING TWO CATEGORICAL EXPLANATORY VARIABLES**
\

Whilst we tend to _group_ points, we need to _split_ boxplots, violin plots and mean, standard error plots, as introducing a new categorical variable creates further subgroups. We use a further `ggplot` layer to do this: facets, which split a plot into subplots.  
 
##### **6.5.1 USING FACETS TO SPLIT PLOTS**
\

<center>
![Facet](www/facet_icon.png){width="50%"}
</center>  
\  

`facet_grid()` specifies rows and columns of subplots to create. The example below adds a line using `facet_grid()` to split the plot into subplots according to `role`. 

1. Create a new data frame which omits NAs and adds a logical variable `shortsighted` which details whether the person is shortsighted or not
2. Create a plot which shows the means and standard errors of `reaction` for each group of `shortsighted`
\
...
data2 <- data %>% 
  na.omit() %>%
    mutate(shortsighted = eyesight < 0)

# create plot
meanSE_plot <- ggplot(data2, aes(x = shortsighted, y = reaction)) + 
...
3. Create another plot which uses `facet_grid()` to split the means and standard errors out by `role`. With the way that `ggplot` layering works, it's not necessary to create a whole new plot. We can just add the facet layer to the plot we just created!  
\

```{r}
# plot with facet split
meanSE_plot + facet_grid(cols = vars(role))
```
(I'm unsure why it's necessary to use the `vars()` function with `facet_grid()`... but it is necessary!)  
\

If we want to colour the points differently, we use the exact same appraoch as for scatter plots. We supply the plotting function (in this case `stat_summary()`) with a colour _aesthetic_. Remember it's an aesthetic because it is linked to a variable.
\

```{r}
meanSE_plot <- ggplot(data2, aes(x = shortsighted, y = reaction)) + 
    stat_summary(fun.data = "mean_se", aes(colour = shortsighted)) +
    theme_minimal() +
...
##### **MEAN AND STANDARD ERROR, BOXPLOTS, AND VIOLIN PLOTS**
\

<center>
![Points](www/pointrange_icon.png){width="20%"}
![Box](www/boxplot_icon.png){width="20%"}
![Violin](www/violinplot_icon.png){width="20%"}
</center>  
\  

Mean and standard error bars are used in the example above, but the same approaches for faceting and colouring apply to boxplots and violin plots. The same approach applies: 

* `facet_grid()` creates facets
  + use the `cols` (or `rows`) argument to create subplots in columns or rows
  + specify the variable in the `vars()` function
* colours are added in the `geom` functions
  + as you're colouring according to a variable, you specify this as an _aesthetic_.  
\

Use the code above and change the aesthetics, geometry and facets to get the following plot:

#### **< EXERCISE 2 >** Box plots
_Edit the code in the exercise below to:_

1. _Add a variable to `data` called `continents_fac` which is `continents` converted to a factor_
2. _Create a box plot which..._
  * _has `continents_fac` as its x aesthetic_
  * _is faceted by `role`_
  * _is coloured by `continents_fac`_
  * _has the x axis label "Number of continents"_
  * _has the y axis label "Number of continents"_
\

```{r ex651_2, exercise=TRUE}
box <- ggplot(data, aes(x = , y = )) +
    geom_boxplot() +
    theme_minimal() + 
    facet_grid()
...
```
```{r ex651_2-solution}
data <- data %>% mutate(continents_fac = as.factor(continents))

box <- ggplot(data, aes(x = continents_fac, y = countries)) +
    geom_boxplot(aes(colour = continents_fac)) +
    theme_minimal() +
    facet_grid(cols = vars(role)) +
    xlab("Number of continents") + ylab("Number of countries")
box
```
\
...
***
### **6.6 VISUALISING TWO NUMERIC EXPLANATORY VARIABLES**
\

<center>
![Points](www/heatmap_icon.png){width="20%"}
![Box](www/contour_icon.png){width="20%"}
![Violin](www/hexmap_icon.png){width="20%"}
</center>  
\  

We won't worry about this now. It's quite a complicated process to plot two continuous explanatory variables (heatmaps, contour plots, or even 3D plots). It's do-able, but for later on in your data journey!  
...
The [_**R Graph Gallery**_](https://www.r-graph-gallery.com/) has advice on how to plot gazillions of different plot types, and links to courses in programming plots.  
\
